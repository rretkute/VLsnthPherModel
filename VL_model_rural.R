...
house<-read.table("Households_rural.txt",header=T)
...
getDogs<-function(){
	dogs<-data.frame(IDs=c(),inf_stat=c(),house=c(), introduced=c(), exposed=c(),infectious=c(), removed=c(), replaced=c())
	id<-1
	for(i in 1:nn){ 
		if(house$dogs[i]>0){
			for(j in 1:house$dogs[i]) {	 
				infstat<-sample(c(1,2,3),1,prob=c(pi_never, pi_low, pi_high))
				   dogs<- data.frame(IDs=c(dogs$IDs, id),inf_stat=c(dogs$inf_stat,infstat), house=c(dogs$house, house$housenum[i]), introduced=c(dogs$introduced, 0), exposed=c(dogs$exposed, -1), infectious=c(dogs$infectious, -1), removed=c(dogs$removed, -1), replaced=c(dogs$replaced,0))
				id<-id+1  }}}  
	return(dogs)
}
...
run_model_br_equilibrium<-function(scal_fl){
	dogs<-getDogs()
	inf0<-sample(dogs$ID[dogs$inf_stat>1], no.inf0)
	dogs$exposed[inf0]<-1
	...
	for (t in 2:(phertime-1)){ 
        ...
        if(length(infHI)+length(infLI)>0){ 
			...
			lambda <- H(house$dogs)*((p_D * VC_D)/pmax(house$dogs, 1))*(p_v_hi* K.HI  +  p_v_li* K.LI)
	...
	# humans 
	if(t>hinftime){
		inf.bites.CH[,t-hinftime]<-H(house$humans)*(total.fl*frc.fl.H/pmax(house$humans, 1))*a_H*a_D* (K.HI  +  K.LI ) * exp(-mu *tau)/mu
		
	}
	prevt.D<-100*(length(dogs$ID[dogs$infectious>0 & dogs$removed<0]))/length(dogs$ID[dogs$removed<0])
	sim.D<-c(sim.D, prevt.D);
	inc.rate.D<-c(inc.rate.D, new.inf.D)
	if(t==phertime-1-yd)	 Lambda<-lambda
	if(t>phertime-1-yd)	Lambda<-Lambda+lambda
 } # t
	return(list(sim.D, inc.rate.D, inf.bites.CH, inf.bites.H, inf.bites.D, house, dogs, FR, Lambda))
}


#  Run model with control
run_model_br_pher<-function(scal_fl, simulEq, housePh){
	sim.D<-simulEq[[1]]; inc.rate.D<-simulEq[[2]]; inf.bites.CH<-simulEq[[3]]; inf.bites.H<-simulEq[[4]]; inf.bites.D<-simulEq[[5]]; house<-housePh; dogs<-simulEq[[7]]; FR<-simulEq[[8]]
	frc.fl.H<-FR[[1]]; frc.fl.O<-FR[[2]]; 
	frc.fl<- frc.fl.H+frc.fl.O	
